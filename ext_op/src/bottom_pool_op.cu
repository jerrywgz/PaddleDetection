...
class BottomPoolGradOpCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    SliceOnAxis<T><<<blocks, threads, 0, dev_ctx.stream()>>>(x->data<T>(), NC_num, height, width, 2, 0, 1, max_val_data);
    ...
    ScatterAddOnAxis<T><<<blocks, threads, 0, dev_ctx.stream()>>>(out_grad->data<T>(), 0, max_ind_data, NC_num, height, width, 2, in_grad_data);

    for (int ind = 1; ind < height; ++ind) {
      UpdateMaxInfo<T><<<blocks, threads, 0, dev_ctx.stream()>>>(x->data<T>(), NC_num, height, width, 2, ind, max_val_data, max_ind_data);
      ScatterAddOnAxis<T><<<blocks, threads, 0, dev_ctx.stream()>>>(out_grad->data<T>(), ind,  max_ind_data, NC_num, height, width, 2, in_grad_data); 
    }
  }
};
