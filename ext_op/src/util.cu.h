...
template <typename T>
__global__  void MaxOut(const T* cur, const T* next, 
...
template <typename T>
__global__  void UpdateMaxInfo(const T* input, const int num, const int index, T* max_val, int* max_ind) {
  CUDA_1D_KERNEL_LOOP(i, num) {
    if (input[i] > max_val[i]) {
      max_val[i] = input[i];
      max_ind[i] = index;
    }
  }
...
template <typename T>
__global__  void ScatterAddOnAxis(const T* grad_in, const int* max_ind, const int* dims, int axis, T* output) {
    ...
    CUDA_1D_KERNEL_LOOP(i, NC_num * length) { 
      ...
      //T val_ = max_val[i];
      if (axis == 2) {
        output[NC_id * HW_num + id_ * dims[3] + length_id] += grad_in[i];
      } else if (axis == 3) {
        output[NC_id * HW_num + length_id * dims[3] + id_] += grad_in[i];
      }
    }
}
