...
def load_dygraph_ckpt(model, pretrain_ckpt=None, resume_ckpt=None, open_debug=False):
    if pretrain_ckpt is not None:
        ...
        for k, v in w_dict.items():
            ...
            for mk in model_keys:
                ...
                if res is not None:
                    if open_debug:
                        print("load: ", k, " rename: ", new_k, v.shape, " --> ", mk,
                              model_state[mk].shape)
                    model_state[mk] = v
                    match_num += 1
                    match_success = True
                    break
        ...
        if open_debug:
            ...
            print("Pretrain weights have {} and match {}".format(len(w_dict.files), match_num))
