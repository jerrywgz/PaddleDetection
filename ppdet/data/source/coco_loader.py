...
def load(anno_path, sample_num=-1, with_background=True, clip_bbox=True):
    """
    ...
    for img_id in img_ids:
        ...
        for inst in instances:
            ...
            x1 = max(0, x)
            y1 = max(0, y)
            x2 = min(im_w - 1, x1 + max(0, box_w - 1)) if clip_bbox else x + box_w
            y2 = min(im_h - 1, y1 + max(0, box_h - 1)) if clip_bbox else y + box_h
