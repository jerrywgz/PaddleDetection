...
def compute_bbox_targets(bboxes1, bboxes2, labels, bbox_reg_weights):
    ...
    targets = np.zeros(bboxes1.shape).astype('float32')
    bbox_reg_weights = np.asarray(bbox_reg_weights).astype('float32')
    targets = bbox2delta(
...
def expand_bbox_targets(bbox_targets_input,
    ...
    bbox_targets = np.zeros(
        (class_labels.shape[0], 4 * class_nums)).astype('float32')
    bbox_inside_weights = np.zeros(bbox_targets.shape).astype('float32')
    for ind in fg_inds:
