...
class CascadeHead(nn.Layer):
    ...
    def forward(self, body_feats=None, rois=None, rois_num=None, inputs=None):
        ...
        """
        if self.training:
            proposals, rois_num, _, targets = self.bbox_assigner(rois, rois_num,
                                                                 inputs)
        ...
        for i in range(self.num_cascade_stages):
            if i > 0:
                proposals = self._get_proposals_from_boxes(pred_bbox,
                                                           inputs['im_shape'])
                if self.training:
                    proposals, rois_num, _, targets = self.bbox_assigner(
                        proposals, rois_num, inputs, i, is_cascade=True)

            rois_feat = self.roi_extractor(body_feats, rois, rois_num)
            #rois_feat = _ScaleGradient.apply(rois_feat, 1.0 / self.num_cascade_stages)
            ...
            pred_bbox = self._get_pred_bbox(deltas, proposals,
                                            self.bbox_weight[i])

        self.assigned_rois = (proposals, rois_num)
        self.assigned_targets = targets

        if self.training:
            loss = {}
            for stage, (scores, deltas, targets, proposals) in enumerate(
                    zip(scores_list, deltas_list, targets_list,
                        proposals_list)):
                loss_stage = self.get_loss(scores, deltas, targets, proposals,
    ...
    def _get_proposals_from_boxes(self, boxes, im_shape):
        proposals = []
