        ...
        for stage in range(num_stages):
            ...
            fc6 = self.add_sublayer(
                ...
                Linear(
                    ...
                    mlp_dim,
                    act='relu',
                    param_attr=ParamAttr(initializer=Xavier(fan_out=fan)),
                    bias_attr=ParamAttr(
...
class BBoxFeat(Layer):
    ...
    def forward(self, body_feats, roi, rois_num, spatial_scale, stage=0):
        rois_feat = self.roi_extractor(body_feats, roi, rois_num, spatial_scale)
...
class BBoxHead(Layer):
    ...
    def forward(self, body_feats, rois, spatial_scale, stage=0):
        roi, rois_num = rois
        bbox_feat = self.bbox_feat(body_feats, roi, rois_num, spatial_scale,
                                   stage)
        bbox_head_out = []
        scores = self.bbox_scores_list[stage](bbox_feat)
        deltas = self.bbox_deltas_list[stage](bbox_feat)
        bbox_head_out.append((scores, deltas))
        return bbox_feat, bbox_head_out

    def _get_head_loss(score, delta, target):
        # bbox cls  
        ...
        loss_bbox_cls = fluid.layers.softmax_with_cross_entropy(
            logits=scores, label=labels_int64)
    ...
    def loss(self, bbox_head_out, targets):
        ...
        for lvl, (bboxhead, target) in enumerate(zip(bbox_head_out, targets)):
            ...
            loss_bbox_cls, loss_bbox_reg = _get_head_loss(score, delta, target)
