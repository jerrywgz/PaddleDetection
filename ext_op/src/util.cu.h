...
__global__  void ScatterAddOnAxis(const T* input, const int start, const int* max_ind, const int NC_num, const int H, const int W, const int axis, T* output) {
  ...
  CUDA_1D_KERNEL_LOOP(i, NC_num * length) { 
    ...
    int id_ = max_ind[i];
    if (axis == 2) {
      input_idx = NC_id * HW_num + start * W + length_id;
      output_idx = NC_id * HW_num + id_ * W + length_id;
    } else if (axis == 3) {
      input_idx = NC_id * HW_num + length_id * W + start;
      output_idx = NC_id * HW_num + length_id * W + id_;
    }
    platform::CudaAtomicAdd(output + output_idx, input[input_idx]);
  }
}
